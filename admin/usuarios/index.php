...
<body>
									...
									<table id="tabela" class="display table" style="width:100%">
										...
										<tbody>
										    ...
										    <tr>
										        ...
										        <td><?=$u->email?></td>
										        <? if($super == 'Sim'){ ?>
										        <td><?=Admin::campo($u->admin, 'nome')?></td>
										        <? } ?>
										        <td><?=$u->status?></td>
										        <td><?=Usuario::gerarBotoes($u->id)?></td>
										    </tr>
										    <? } } ?>
										    <? } else { ?>
										    <? 
										        $usuarios = Usuario::find(0, array("admin = '".$_SESSION['admin_id']."' OR admin = '' OR admin = 0"));
										        if(count($usuarios) >=1){
										            foreach($usuarios as $u){
										    ?>
										    <tr>
    ...
    <script src="banners/js/banners.js"></script>
