...
$usuarios = Usuario::find(0, array("evolution = 'Sim'"));
if(count($usuarios) >=1){
    $usuario = $usuarios[0];
    $usuario_id = $usuario->id;
    $entradas = EntradaEvolution::find(0, array("confirmada = '' AND status = '' AND usuario = '".$usuario_id."'"));
    foreach($entradas as $entrada){
        if($entrada->data_cadastro < $passado){
            $e = EntradaEvolution::find($entrada->id);
            $e->confirmada = 'n'; 
            $e->status = 'n';
            $e->save();
            deleteMessage($entrada->id);
        } else {
            $estrategias = EstrategiaEvolution::find(0, array("numero = '".$entrada->estrategia."' AND usuario = '".$usuario_id."'"));
            $estrategia = $estrategias[0];
            $analisa = intval($estrategia->analisa);
            $confirma = intval($estrategia->confirma);
            $paginate = $confirma - $analisa;
            
            $resultados = ResultadoEvolution::paginate(0, $paginate, "resultado_id > '".$entrada->analisado_id."' AND roleta_id = '".$entrada->roleta_id."'", "resultado_id ASC");
            if(count($resultados) >0){
                if(count($resultados) <= $paginate){
                    ...
                    if($estrategia->abortar == 'Sim'){
                        ...
                        if($zero == 0){
                            ...
                            if($entrada->estrategia == 25){
                                $analisados = ResultadoEvolution::find(0, array("resultado_id = '".$entrada->analisado_id."'"));
                    ...
                    if($contagem == 0){
                        ...
                        $numeros = ResultadoEvolution::paginate(0, 3, "resultado_id <= '".$resultados[0]->resultado_id."' AND roleta_id = '".$entrada->roleta_id."'", "resultado_id DESC");
                        if(count($numeros)  >=1){
...
function enviaSinal($id, $sequencia){
    ...
    $roletas = RoletaEvolution::find(0, array("numero = '".$entrada->roleta_id."' AND usuario = '".$entrada->usuario."'"));
    $estrategias = EstrategiaEvolution::find(0, array("numero = '".$entrada->estrategia."' AND usuario = '".$entrada->usuario."'"));
    $estrategia = $estrategias[0];
    $msg = MensagemEvolution::find(0, array("usuario = '".$entrada->usuario."'"));
    $mensagens = $msg[0];
    ...
    $EntradaGrupo = EntradaGrupoEvolution::find(0, array("entrada = '".$id."' AND tipo = 'analisa'"));
