...
if(count($usuarios) >=1){
    ...
    $entradas = EntradaEvolution::find(0, array("confirmada = 's' AND status = '' AND usuario = '".$usuario_id."'"), 'id DESC');
    foreach($entradas as $entrada){
        if($entrada->data_cadastro < $passado){
            ...
            $numeros = array();
            $red = 0;
            $resultados = ResultadoEvolution::paginate(0, $paginate, "resultado_id > '".$entrada->confirmado_id."' AND roleta_id = '".$entrada->roleta_id."'", "resultado_id ASC");
                ...
                foreach($resultados as $resultado){
                    if($resultado->cor == 'G' && $estrategia->zero == 'Sim'){
                        ...
                        if($estrategia->numero == 25){
                            $ultimoNumero = ResultadoEvolution::find(0, array("resultado_id = '".$entrada->confirmado_id."'"));
...
function marcaGreen($resultado_id, $numero, $entrada_id, $numeros){
    $e = EntradaEvolution::find($entrada_id);
    $e->status = 'g';
...
function enviaSinalRed($id, $numeros){
    ...
    $roletas = RoletaEvolution::find(0, array("id = '".$entrada->roleta_id."'"));
    ...
    $EntradaGrupo = EntradaGrupoEvolution::find(0, array("entrada = '".$id."' AND tipo = 'confirma'"));
...
function sendMessage($chatID, $mensagem, $id, $msg_id, $marcacao){
    ...
    if($marcacao == 'Responder'){
        ...
        $entradaGrupo = EntradaGrupoEvolution::find(0, array("entrada = '".$id."' AND msg_id = '".$msg_id."'"));
